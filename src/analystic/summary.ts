...
export class Summary {
  ...
  constructor(private readonly rootSceneProxy: ElementProxy<RootScene>) {
    ...
    GlobalEvent
      .on('@app/element-proxy/before:exec', (elemProxy: ElementProxy<any>) => {
        if (elemProxy.$ instanceof RootScene) {
          this.#time.execution = Date.now()
        } else {
          this.#count.exec++
        }
      })
      .on('@app/element-proxy/after:dispose', (elemProxy: ElementProxy<any>) => {
        if (elemProxy.$ instanceof RootScene) {
